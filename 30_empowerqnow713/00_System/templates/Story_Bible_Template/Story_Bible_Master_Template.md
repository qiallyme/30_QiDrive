---
title: Story Bible Master Template
content_project:
content_project_type: series
status: active
umbrella: EmpowerQNow
platform: QSaysIt
created:
updated:
tags:
  - story-bible
layout: page
slug: story-bible-master-template
summary: ""
created_at: "2026-07-16T06:19:39-04:00"
updated_at: "2026-07-18T11:03:16-04:00"
author: ""
owner: ""
keywords: []
aliases: []
context: ""
sensitivity: internal
classification: business_internal
realm_label: ""
uid: 826bd39685a74ccfb84adb9210cf1d0a
canonical_ref: ""
source_type: manual
template_key: master-template
type: note
---

# Story Bible

## 1. Project Identity

**Project name:**  
**Project type:** Series / Book / Other  
**Umbrella:** EmpowerQNow  
**Primary platform:** QSaysIt  
**One-sentence identity:**  

## 2. Core Thesis

What central truth, question, tension, or idea governs this project?

## 3. Purpose

Why does this project exist?

What should it help the reader see, question, understand, feel, or do?

## 4. Manifesto

Write the project's governing declaration.

This should explain what the project believes, refuses, protects, challenges, and makes possible.

## 5. Audience

**Primary audience:**  
**Secondary audience:**  
**What they are carrying:**  
**What they need from this work:**  

## 6. Themes and Motifs

### Primary themes

- 
- 
- 

### Recurring motifs, symbols, or phrases

- 
- 
- 

### Questions the project repeatedly asks

- 
- 
- 

## 7. Voice and Style

**Voice:**  
**Tone:**  
**Emotional range:**  
**Structural habits:**  
**Language to favor:**  
**Language to avoid:**  

## 8. Canon and Boundaries

### Canonical truths

- 
- 
- 

### Things this project does not claim

- 
- 
- 

### Continuity rules

- 
- 
- 

## 9. Structure

### For a series

- Series premise
- Recurring formats
- Installment categories
- Recommended reading order, if any

### For a book

- Book premise
- Part structure
- Chapter order
- Narrative or argumentative arc

## 10. Content Map

| ID | Working title | Type | Status | Core idea | Related theme |
|---|---|---|---|---|---|
| 001 |  |  | seed |  |  |

## 11. Seed Bank

### Raw seeds

- 

### Developed seeds

- 

### Unresolved questions

- 

## 12. Research and Sources

### Project-specific sources

- 

### Shared EmpowerQNow sources

- 

### Sources requiring verification

- 

## 13. Glossary

| Term | Meaning in this project |
|---|---|
|  |  |

## 14. Connections

### Related series

- 

### Related books

- 

### Shared frameworks

- 

### Possible companion posts

- 

## 15. Publishing Notes

**Primary destination:**  
**Series/category label:**  
**Expected formats:**  
**Cross-posting notes:**  
**Media needs:**  

## 16. Continuity Log

| Date | Decision or change | Reason | Files affected |
|---|---|---|---|
|  |  |  |  |
